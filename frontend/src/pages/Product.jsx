...
const Product = () => {
  ...
  useEffect(() => {
    if (productDetails && productDetails.baseImage) {
      ...
      setSize(productDetails.variants[0].size)
    }
  }, [productDetails]); ``
    ...
    <div className="max-w- mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12 transition-opacity duration-500">
      ...
      <div className="flex flex-col lg:flex-row gap-8 xl:gap-12">
        ...
        <div className="flex-1 flex flex-col-reverse md:flex-row gap-4">
          ...
          <div className="flex md:flex-col gap-2 overflow-x-auto md:overflow-y-auto justify-start md:w-24 w-full pb-2 md:pb-0" style={{ maxHeight: '500px' }}>
            <img
              ...
              className={`w-20 h-20 md:w-full md:h-24 object-cover rounded-lg border-2 cursor-pointer transition-opacity duration-700 ease-in-out ${image === productDetails.baseImage ? 'border-blue-500' : 'border-transparent'}`}
            ...
            {productDetails?.images?.map((item, index) => (
              <img
                ...
                src={item}
                key={index}
                className={`w-20 h-20 md:w-full md:h-24 object-cover rounded-lg border-2 cursor-pointer transition-opacity duration-700 ease-in-out ${image === item ? 'border-blue-500' : 'border-transparent'}`}
        ...
        <div className="flex-1">
            ...
            <div className="grid grid-cols-4 sm:grid-cols-6 gap-3">
              {productDetails.variants.map((item, index) => (
                <button
                  ...
                  className={`py-3 px-2 text-center rounded-lg border transition-all duration-300 ${item.size === size
          ...
          <div className="flex flex-col sm:flex-row items-center sm:space-x-6 space-y-4 sm:space-y-0 mt-8 w-fit">
          ...
          ${!size ? 'border pl-2 pr-2 text-black' : ''}
        `}
            >
              {size ? <AddToCartButton handleCart={handleCart} state={addedToCart} /> : 'SELECT A SIZE'}
            </button>
          </div>
      ...
      <div className="mt-16">
        <div className="flex border-b">
          <button
            ...
            className={`px-6 py-3 font-medium text-sm transition-colors ${activeTab === 'description'
